/**
 * Тестирование API вызова с новым подходом, основанным на документации
 * Используем singular dimension и правильную фильтрацию
...
// Тестируем вызов API для FBO с новым подходом
async function testOzonSalesFBO() {
  console.log('🔍 Тестируем API вызов для SKU 301916350 (FBO) с новым подходом за период 26.01.2026-26.02.2026...');
  ...
  // Подготовим тело запроса для FBO с новым подходом (singular dimension)
  const body = {
    date_from: formatDateTimeForAPI(dateRange.start),
    date_to: formatDateTimeForAPI(dateRange.end),
    dimension: ["sku"], // Singular dimension как в документации
  ...
  console.log('📋 Тело запроса FBO (новый подход):', JSON.stringify(body, null, 2));
      ...
      items.forEach((item, index) => {
        ...
        if (sku === "301916350") {
          ...
          if (item.metrics && Array.isArray(item.metrics) && item.metrics.length > 0) {
            ...
            console.log(`   ✅ SKU ${sku}, Units sold: ${metricValue}`);
...
// Тестируем вызов API для FBS с новым подходом
async function testOzonSalesFBS() {
  console.log('\n🔍 Тестируем API вызов для SKU 301916350 (FBS) с новым подходом за период 26.01.2026-26.02.2026...');

  const dateRange = getSpecificDateRange();
  console.log(`📅 Диапазон дат: ${formatDateTimeForAPI(dateRange.start)} → ${formatDateTimeForAPI(dateRange.end)}`);

  // Подготовим тело запроса для FBS с новым подходом (singular dimension)
  const body = {
    date_from: formatDateTimeForAPI(dateRange.start),
    date_to: formatDateTimeForAPI(dateRange.end),
    dimension: ["sku"], // Singular dimension как в документации
  ...
  console.log('📋 Тело запроса FBS (новый подход):', JSON.stringify(body, null, 2));
...
async function runTests() {
  ...
  console.log(`\n🏁 ФИНАЛЬНЫЕ РЕЗУЛЬТАТЫ (новый подход):`);
  ...
  // Также проверим, если хотя бы одно значение близко к ожидаемому
  const fboClose = Math.abs(fboResult.fbo - 40) <= 5; // в пределах 5 единиц
  const fbsClose = Math.abs(fbsResult.fbs - 5) <= 2;  // в пределах 2 единиц

  if (fboClose && fbsClose) {
    console.log(`🎯 Тем не менее, оба значения находятся в приемлемом диапазоне.`);
  } else if (fboClose) {
    console.log(`🎯 Значение FBO близко к ожидаемому.`);
  } else if (fbsClose) {
    console.log(`🎯 Значение FBS близко к ожидаемому.`);
  }
}
