/**
 * Тестирование API вызова для известного SKU из предыдущих успешных ответов
 * Проверим SKU 1058595888, который показывал 4421 продажу
 */
...
// Тестируем вызов API для FBO с известным SKU
async function testKnownSKU() {
  console.log('🔍 Тестируем API вызов для известного SKU 1058595888...');
  ...
  // Подготовим тело запроса для FBO с известным SKU
  const body = {
    ...
    filters: [
      ...
      },
      {
        column: "sku",
        operation: "in",
        values: ["1058595888"]  // Известный SKU
      }
    ],
    limit: 1000
  };
  ...
  try {
    ...
    if (response.data && response.data.result && response.data.result.data) {
      ...
      console.log(`\n📈 Всего найдено для SKU 1058595888: ${total}`);
    }
  } catch (error) {
