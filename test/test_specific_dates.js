/**
 * Тестирование API с конкретными датами 26.01.2026-26.02.2026
 * для проверки значений FBO 40 и FBS 5 для SKU 301916350
 */
...
// Тестируем вызов API для FBO с конкретным SKU и датами
async function testOzonSalesFBOWithSpecificDates() {
  console.log('🔍 Тестируем API вызов для SKU 301916350 (FBO) с датами 26.01.2026-26.02.2026...');
  ...
  // Подготовим тело запроса для FBO с конкретным SKU
  const body = {
  ...
  console.log('📋 Тело запроса FBO (с конкретными датами и SKU):', JSON.stringify(body, null, 2));
  ...
  try {
    ...
    if (response.data && response.data.result && response.data.result.data) {
      ...
      console.log(`\n🎯 Результаты для SKU 301916350 (FBO) с датами 26.01.2026-26.02.2026: ${totalFBO} единиц`);

      return { fbo: totalFBO, found: foundTargetSku, count: items.length };
    }
  } catch (error) {
    console.error('❌ Ошибка при вызове API (FBO) с конкретными датами:');
    if (error.response) {
...
// Тестируем вызов API для FBS с конкретным SKU и датами
async function testOzonSalesFBSWithSpecificDates() {
  console.log('\n🔍 Тестируем API вызов для SKU 301916350 (FBS) с датами 26.01.2026-26.02.2026...');
  ...
  // Подготовим тело запроса для FBS с конкретным SKU
  const body = {
    date_from: formatDateTimeForAPI(dateFrom),
    date_to: formatDateTimeForAPI(dateTo),
    dimension: ["sku"], // Размерность по SKU (используем правильное имя параметра как в документации)
  ...
  console.log('📋 Тело запроса FBS (с конкретными датами и SKU):', JSON.stringify(body, null, 2));
  ...
  try {
    ...
    if (response.data && response.data.result && response.data.result.data) {
      ...
      console.log(`\n🎯 Результаты для SKU 301916350 (FBS) с датами 26.01.2026-26.02.2026: ${totalFBS} единиц`);

      return { fbs: totalFBS, found: foundTargetSku, count: items.length };
    }
  } catch (error) {
    console.error('❌ Ошибка при вызове API (FBS) с конкретными датами:');
    if (error.response) {
      console.error('Статус:', error.response.status);
      console.error('Ответ:', error.response.data);
    } else if (error.request) {
...
async function runTests() {
  ...
  const fboResult = await testOzonSalesFBOWithSpecificDates();
  const fbsResult = await testOzonSalesFBSWithSpecificDates();
