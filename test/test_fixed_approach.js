/**
 * Тестирование исправленного подхода для API вызова с правильной структурой dimensions
...
// Тестируем вызов API для FBO с исправленным подходом
async function testOzonSalesFBO() {
  console.log('🔍 Тестируем API вызов для SKU 301916350 (FBO) с исправленным подходом...');
  ...
  // Подготовим тело запроса для FBO с правильной структурой dimensions
  const body = {
  ...
  console.log('📋 Тело запроса FBO (исправленный подход):', JSON.stringify(body, null, 2));
  ...
  try {
    ...
    if (response.data && response.data.result && response.data.result.data) {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        // Ищем целевой SKU и правильный fulfillment_type в размерностях
        let sku = null;
...
// Тестируем вызов API для FBS с исправленным подходом
async function testOzonSalesFBS() {
  console.log('\n🔍 Тестируем API вызов для SKU 301916350 (FBS) с исправленным подходом...');

  const dateRange = getSpecificDateRange();
  console.log(`📅 Диапазон дат: ${formatDateTimeForAPI(dateRange.start)} → ${formatDateTimeForAPI(dateRange.end)}`);

  // Подготовим тело запроса для FBS с правильной структурой dimensions
  const body = {
    date_from: formatDateTimeForAPI(dateRange.start),
    date_to: formatDateTimeForAPI(dateRange.end),
    dimensions: ["sku", "fulfillment_type"],
  ...
  console.log('📋 Тело запроса FBS (исправленный подход):', JSON.stringify(body, null, 2));
...
async function runTests() {
  console.log('🚀 Запускаем тесты с исправленным подходом...');
  console.log('💡 Этот подход использует правильную структуру dimensions как в документации');

  const fboResult = await testOzonSalesFBO();
  const fbsResult = await testOzonSalesFBS();

  console.log(`\n🏁 ФИНАЛЬНЫЕ РЕЗУЛЬТАТЫ (исправленный подход):`);
  ...
  if (fboResult.found || fbsResult.found) {
    console.log(`\n✅ SKU 301916350 присутствует в данных API с правильными значениями`);
  } else {
    console.log(`\n❌ SKU 301916350 не найден в данных API за указанный период с правильным типом исполнения`);
