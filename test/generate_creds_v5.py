import json

# Taking the EXTREMELY ORIGINAL JSON from the user's first message to avoid any truncation or mangling.
# We will use triple quotes to hold the raw string.

raw_json = r"""{
  "type": "service_account",
  "project_id": "seo-pipline",
  "private_key_id": "f87f76243a9a9d363e5a064bb54c3674744b3553",
  "private_key": "-----BEGIN PRIVATE KEY-----\\ly9FmYaYA25qQFw+TL4vnSU9xQrU17AH8oUJsIKdpK72K5pYZZRipjQtniy0GH1p\nZ1rLXuPIiN3aQY6mN6VspMrswwKBgQDJX29fPt8b2cUGfUxeqblyvGGgirdjP0+a\nb0DBkoXXKg6iEEKrwLrB/JySbRsji80Y/Nn+41porYeaTV0n0y37eZZ+vS4aEOcb\\n-----END PRIVATE KEY-----\n",
  "client_email": "seo-pipline@seo-pipline.iam.gserviceaccount.com",
  "client_id": "116216456011406269643",
  "auth_uri": "https://accounts.google.com/o/oauth2/auth",
  "token_uri": "https://oauth2.googleapis.com/token",
  "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
  "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/seo-pipline%40seo-pipline.iam.gserviceaccount.com",
  "universe_domain": "googleapis.com"
}"""

# Load it to verify it's valid JSON
data = json.loads(raw_json)

# Save it to the file
with open("gsheets_creds.json", "w", encoding="utf-8") as f:
    json.dump(data, f, indent=2)

print("gsheets_creds.json has been perfectly reconstructed from the original source.")
