...
function updateWBAnalytics() {
  ...
  const monthSalesColumn = columnByHeader_(sheet, "Уход Мес ВБ");
  const quarterSalesColumn = columnByHeader_(sheet, "Уход КВ ВБ");
  ...
  // R (18): Уход Мес ВБ
  sheet.getRange(2, monthSalesColumn, monthValues.length, 1).setValues(monthValues);

  // S (19): Уход КВ ВБ
  sheet.getRange(2, quarterSalesColumn, quarterValues.length, 1).setValues(quarterValues);
