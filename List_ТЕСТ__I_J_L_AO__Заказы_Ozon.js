...
function startFetchAndWriteAnalytics() {
  return runWithTelegramAlertGAS_('startFetchAndWriteAnalytics', startFetchAndWriteAnalytics_);
}
  ...
  const periods = [
    {
      ...
      write: function (metricsMap) {
        ...
        sheet.getRange(2, columnByHeader_(sheet, 'Уход Мес ОЗОН'), orderedUnitsMonthList.length, 1).setValues(orderedUnitsMonthList);
        sheet.getRange(2, columnByHeader_(sheet, 'Сумма заказов Мес ОЗОН'), revenueMonthList.length, 1).setValues(revenueMonthList);
      }
    },
    {
      label: "Квартал",
      ...
      write: function (metricsMap) {
        ...
        sheet.getRange(2, columnByHeader_(sheet, 'Уход КВ'), orderedUnitsQuarterList.length, 1).setValues(orderedUnitsQuarterList);
      }
    },
    {
      label: "Год",
      ...
      write: function (metricsMap) {
        ...
        sheet.getRange(2, columnByHeader_(sheet, 'Сумма заказов Год ОЗОН'), revenueYearList.length, 1).setValues(revenueYearList);
      }
    }
  ];
...
function continueFetchAndWriteAnalytics() {
  return runWithTelegramAlertGAS_('continueFetchAndWriteAnalytics', continueFetchAndWriteAnalytics_);
}
