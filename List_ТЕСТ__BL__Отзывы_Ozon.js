 ...
 * Запись: BL (64) — "Отзывы".
 *
 * Плановый полный проход теперь выполняет ozon_reviews_local.js. Ниже
 * сохранён ручной аварийный вариант на случай недоступности локального задания.
 */
...
function updateOzonReviewCountBL() {
  return runWithTelegramAlertGAS_('updateOzonReviewCountBL', updateOzonReviewCountBL_);
}
...
function updateOzonReviewCountBLManual_() {
  ...
  const reviewColumn = columnByHeader_(sheet, 'Отзывы');

  if (!uniqueSkus.length) {
    sheet.getRange(2, reviewColumn, skuValues.length, 1).setValue(0);
...
function resumeOzonReviewCountBL() {
  return runWithTelegramAlertGAS_('resumeOzonReviewCountBL', resumeOzonReviewCountBL_);
}

function resumeOzonReviewCountBL_() {
  processOzonReviewCountPages_();
}
...
function processOzonReviewCountPages_() {
  ...
  while (Date.now() - startedAt < OZON_REVIEW_COUNT_MAX_RUN_MS) {
    ...
    if (responseCode < 200 || responseCode >= 300) {
      saveOzonReviewCountMap_(countMap);
      saveOzonReviewCountState_(state);
      throw new Error("Ozon review/list: HTTP " + responseCode + " " + text.substring(0, 500));
    }
...
function finishOzonReviewCount_() {
  ...
  sheet.getRange(2, columnByHeader_(sheet, 'Отзывы'), valuesToWrite.length, 1).setValues(valuesToWrite);
