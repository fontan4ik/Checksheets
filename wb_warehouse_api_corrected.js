...
class WildberriesWarehouseAPI {
  ...
  async getWarehouses() {
    ...
    try {
      ...
      // Проверяем формат ответа: API может возвращать массив напрямую
      let warehouses = [];
      ...
      if (warehouses.length > 0) {
        console.log(`✅ Успешно получено ${warehouses.length} складов`);

        // Выводим информацию о каждом складе
        warehouses.forEach((warehouse, index) => {
          console.log(
            `  ${index + 1}. ${warehouse.name} (ID: ${warehouse.id}, Office ID: ${warehouse.officeId || "N/A"}, Cargo Type: ${warehouse.cargoType || "N/A"})`,
          );

          if (warehouse.deliveryType) {
            console.log(`      Тип доставки: ${warehouse.deliveryType}`);
          }

          console.log(
            `      Обрабатывается: ${warehouse.isProcessing ? "Да" : "Нет"}`,
          );
        });
      } else {
        console.warn(
          "⚠️ Неожиданный формат ответа или пустой список складов:",
          response,
        );
      }

      // Возвращаем как объект с полем warehouses для совместимости
  ...
  async diagnoseWarehouses(dateParams = {}) {
    ...
    try {
      ...
      const warehousesResult = await this.getWarehouses();

      if (!warehousesResult || !warehousesResult.warehouses) {
      ...
      // Показываем основные склады (попробуем искать по известным названиям)
      const mainWarehouseNames = ["Москва", "СПБ", "Казань", "Екат", "Колед"];
      const mainWarehouses = warehouses.filter((wh) =>
        mainWarehouseNames.some((name) =>
          wh.name.toLowerCase().includes(name.toLowerCase()),
        ),
      );

      if (mainWarehouses.length > 0) {
        console.log("\n📌 Основные склады (по названию):");
        mainWarehouses.forEach((warehouse, index) => {
          console.log(
            `  ${index + 1}. ${warehouse.name} (ID: ${warehouse.id}, Office ID: ${warehouse.officeId})`,
          );
        });
      } else {
        // Показываем первые 5 складов если не нашли "основные"
        console.log("\n📌 Первые 5 складов:");
        warehouses.slice(0, 5).forEach((warehouse, index) => {
          console.log(
            `  ${index + 1}. ${warehouse.name} (ID: ${warehouse.id}, Office ID: ${warehouse.officeId})`,
          );
        });
