...
function updateOzonPriceIndexBK() {
  return runWithTelegramAlertGAS_('updateOzonPriceIndexBK', updateOzonPriceIndexBK_);
}

function updateOzonPriceIndexBK_() {
  const sheet = mainSheet();
  const indexColumn = columnByHeader_(sheet, 'Индекс');
  ...
  if (!offerIds.length) {
    sheet.getRange(2, indexColumn, offerValues.length, 1).setValue("без индекса");
  ...
  sheet.getRange(2, indexColumn, valuesToWrite.length, 1).setValues(valuesToWrite);
