...
/** Resolve a write destination by its visible row-1 header; never guess a column. */
function columnByHeader_(sheet, header) {
  ...
  if (!wanted) throw new Error("Пустое имя колонки для листа " + sheet.getName());
  const headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
  const matches = headers.reduce((columns, value, index) => {
    if (normalize(value) === wanted) columns.push(index + 1);
    return columns;
  }, []);
  if (matches.length !== 1) {
    throw new Error('Лист "' + sheet.getName() + '": заголовок "' + header +
      '" найден ' + matches.length + ' раз; колонки: ' + matches.join(", "));
  }
  return matches[0];
}
