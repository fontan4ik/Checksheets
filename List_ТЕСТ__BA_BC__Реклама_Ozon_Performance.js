...
const OZON_PERFORMANCE_BASE_URL_V3 = "https://api-performance.ozon.ru";
...
function getPerfToken() {
  ...
  const response = UrlFetchApp.fetch(OZON_PERFORMANCE_BASE_URL_V3 + '/api/client/token', {
    method: 'post',
...
function getPerfCampaigns(token) {
  const response = UrlFetchApp.fetch(OZON_PERFORMANCE_BASE_URL_V3 + '/api/client/campaign', {
    method: 'get',
...
function createPerfReport(token, campaignIds, dateFrom, dateTo) {
  ...
  const response = UrlFetchApp.fetch(OZON_PERFORMANCE_BASE_URL_V3 + '/api/client/statistics', {
    method: 'post',
    contentType: 'application/json',
    headers: { 'Authorization': 'Bearer ' + token },
    payload: JSON.stringify({
...
function waitPerfReport(token, uuid) {
  ...
  const url = OZON_PERFORMANCE_BASE_URL_V3 + '/api/client/statistics/' + uuid;
  ...
  for (let i = 0; i < 25; i++) {
    ...
    if (data.state === 'OK' && data.link) {
      const downloadUrl = OZON_PERFORMANCE_BASE_URL_V3 + data.link;
...
function updateOzonAdPerfFinal() {
  return runWithTelegramAlertGAS_("updateOzonAdPerfFinal", updateOzonAdPerfFinalCore_);
}

function updateOzonAdPerfFinalCore_() {
  ...
  try {
    ...
    sheet.getRange(2, columnByHeader_(sheet, "Реклама Количество"), ba.length, 1).setValues(ba);
    sheet.getRange(2, columnByHeader_(sheet, "Реклама Стоимость, ₽"), bb.length, 1).setValues(bb);
    sheet.getRange(2, columnByHeader_(sheet, "Реклама Расход, ₽"), bc.length, 1).setValues(bc);
