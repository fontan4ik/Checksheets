...
const ODC_GAUSS_SHEET_NAME = "GAUSS TR";
const ODC_GAUSS_WAREHOUSE_NAME = "Gauss МСК";
const ODC_FERON_WAREHOUSE_NAME = "ФЕРОН";  // Ищет "ФБС ФЕРОН МОСКВА"
 ...
 */
function readODCStocksFromSheet() {
  const spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = spreadsheet.getSheetByName(ODC_GAUSS_SHEET_NAME);

  if (!sheet) {
    Logger.log(`❌ Лист "${ODC_GAUSS_SHEET_NAME}" не найден!`);
    return [];
  }
  ...
  if (lastRow < 2) {
    Logger.log(`❌ Нет данных на листе "${ODC_GAUSS_SHEET_NAME}"`);
    return [];
  }
  ...
  Logger.log(`📊 Прочитано ${stocks.length} товаров из листа "${ODC_GAUSS_SHEET_NAME}"`);
 ...
 */
function getODCGaussWarehouseId() {
  Logger.log(`🔍 Поиск склада "${ODC_GAUSS_WAREHOUSE_NAME}"...`);

  const warehouses = getODCOzonWarehouses();
  ...
  if (!gaussWarehouse) {
    Logger.log(`❌ Склад "${ODC_GAUSS_WAREHOUSE_NAME}" не найден!`);
 ...
 * @returns {number|null} ID склада или null
 */
function getODCFeronWarehouseId() {
  Logger.log(`🔍 Поиск склада "${ODC_FERON_WAREHOUSE_NAME}"...`);

  const warehouses = getODCWBWarehouses();
  ...
  if (!feronWarehouse) {
    Logger.log(`❌ Склад "${ODC_FERON_WAREHOUSE_NAME}" не найден!`);
    Logger.log("Доступные склады:");
    warehouses.forEach(w => {
 ...
 * @param {number} warehouseId - ID склада
 */
function updateODCWBStocks(stocks, warehouseId) {
...
function syncStocksFromSheetsSkipODC() {
  ...
  Logger.log(`📊 Шаг 1: Чтение данных из листа "${ODC_GAUSS_SHEET_NAME}"...`);
  const stocks = readODCStocksFromSheet();
  ...
  const gaussWarehouseId = getODCGaussWarehouseId();
  const feronWarehouseId = getODCFeronWarehouseId();
  ...
  if (gaussWarehouseId) {
    ...
    updateODCOzonStocks(stocks, gaussWarehouseId);
  }
...
function testStockSyncSkipODC() {
  ...
  const gaussId = getODCGaussWarehouseId();
  const feronId = getODCFeronWarehouseId();
