...
function updateOzonNTCSalesYesterday() {
  return runWithTelegramAlertGAS_('updateOzonNTCSalesYesterday', updateOzonNTCSalesYesterday_);
}

function updateOzonNTCSalesYesterday_() {
  const sheet = mainSheet();
  const lastRow = sheet.getLastRow();

  const salesColumn = columnByHeader_(sheet, OZON_NTC_SALES_HEADER);
  if (lastRow < 2) {
  ...
  sheet.getRange(2, salesColumn, output.length, 1).setValues(output);
