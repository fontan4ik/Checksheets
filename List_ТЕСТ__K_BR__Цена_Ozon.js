function getOzonPricesOptimized() {
  return runWithTelegramAlertGAS_('getOzonPricesOptimized', getOzonPricesOptimized_);
}

function getOzonPricesOptimized_() {
  const sheet = mainSheet();
  const lastRow = sheet.getLastRow();
  const minPriceColumn = columnByHeader_(sheet, 'Цена мин. бустинга Ozon');
  ...
  if (!completeSuccess) {
    throw new Error('Ozon цены: выгрузка завершилась с ошибками API (' + apiErrorCount + '); предыдущие цены сохранены');
  }
  ...
  if (numRows > 0) {
    sheet.getRange(startRow, columnByHeader_(sheet, 'ЦЕНА ОЗОН'), numRows, 1).setValues(pricesToWrite);
  }
  ...
  if (!completeSuccess) {
    throw new Error('Ozon min elastic boost: не удалось обновить минимальные цены (' + apiErrorCount + ' ошибок)');
  }
}
