 ...
 *
 * Читает остатки из листа "ARL TR":
 * - Ozon: Арлайт Москва
 * - WB: остаток попадает в StreamSupps!M, затем в итоговую AC;
 *   на склад 1449484 его отправляет sync-feron-stocks.js.
 *
...
const ARL_OZON_BASE_DELAY = 1000;                 // Базовый delay для Ozon retry (429/transport)
const ARL_WB_BASE_DELAY = 3000;                   // Базовый delay для WB retry (429/transport)
const ARL_OZON_MAX_RETRIES = 3;
const ARL_WB_MAX_RETRIES = 3;
...
/**
 * Главная точка входа вынесена первой в файле, чтобы Apps Script UI
 * по умолчанию выбирал именно syncARLStocks, а не readARLStocksFromSheet.
 */
function syncARLStocks() {
  return syncARLStocksImpl();
}
...
function fetchARLOzonWarehouseStocks(offerIds, warehouseId) {
  const stockMap = {};
  const chunkSize = 500;
  let lastRequestTime = Date.now() - 1000 / RPS();
...
function verifyARLOzonStocks(stocks, warehouseId) {
  const expected = stocks.filter(s => s.offer_id);
  if (expected.length === 0) {
    return;
  }

  const actualMap = fetchARLOzonWarehouseStocks(expected.map(s => String(s.offer_id)), warehouseId);
  const mismatches = [];
...
function sendARLWBStocksBatch(batch, warehouseId, retryCount = 0) {
  ...
  if (!response) {
    if (retryCount < ARL_WB_MAX_RETRIES) {
      const delay = ARL_WB_BASE_DELAY * Math.pow(2, retryCount);
      Logger.log(`⏳ WB transport error: ожидание ${delay/1000} сек перед retry ${retryCount + 1}/${ARL_WB_MAX_RETRIES}...`);
      Utilities.sleep(delay);
      return sendARLWBStocksBatch(batch, warehouseId, retryCount + 1);
    }
    Logger.log(`⏭️ WB transport error: пропуск после ${ARL_WB_MAX_RETRIES} попыток`);
  ...
  if (code === 429 && retryCount < ARL_WB_MAX_RETRIES) {
    const delay = ARL_WB_BASE_DELAY * Math.pow(2, retryCount);
    Logger.log(`⏳ WB 429: ожидание ${delay/1000} сек перед retry ${retryCount + 1}/${ARL_WB_MAX_RETRIES}...`);
    Utilities.sleep(delay);
    return sendARLWBStocksBatch(batch, warehouseId, retryCount + 1);
  }

  if (code === 429) {
    Logger.log(`⏭️ WB 429: пропуск после ${ARL_WB_MAX_RETRIES} попыток`);
...
function processARLWBConflictIndividually(validBatch, warehouseId, batchLabel) {
  ...
  Logger.log(`🔍 ${batchLabel}: бинарное дробление ODC/CD+ (${validBatch.length} шт)...`);

  const processChunk = (chunk, label, depth) => {
    if (!chunk || chunk.length === 0) {
      return;
    }

    // На одиночном товаре 409 ODC/CD+ означает, что товар нельзя выгрузить на этот склад.
    if (chunk.length === 1) {
      const item = chunk[0];
      ...
      if (result.cargoRestriction) {
        Logger.log(`⏸️ ${label}: пропущен ODC/CD+ chrtId=${item.chrtId}, amount=${item.amount}`);
        skippedCount++;
        return;
      }

      Logger.log(`❌ ${label}: ошибка для chrtId=${item.chrtId}, code=${result.code}`);
      errorCount++;
      return;
    }

    const result = sendARLWBStocksBatch(chunk, warehouseId);

    if (result.ok) {
      successCount += chunk.length;
      Logger.log(`✅ ${label}: chunk ${chunk.length} товаров обработан`);
      return;
    }

    if (result.cargoRestriction) {
      const mid = Math.ceil(chunk.length / 2);
      Logger.log(`↔️ ${label}: ODC/CD+, делю ${chunk.length} на ${mid} + ${chunk.length - mid}`);
      processChunk(chunk.slice(0, mid), `${label}.1`, depth + 1);
      Utilities.sleep(500);
      processChunk(chunk.slice(mid), `${label}.2`, depth + 1);
      return;
    }

    Logger.log(`❌ ${label}: ошибка API для chunk ${chunk.length}, code=${result.code}`);
    if (result.text) {
      Logger.log(result.text.substring(0, 300));
    }
    errorCount += chunk.length;
  };

  processChunk(validBatch, batchLabel, 0);

  Logger.log(`📊 ${batchLabel}: binary split ✅ ${successCount}, ⏸️ ${skippedCount}, ❌ ${errorCount}`);
...
function syncARLStocksImpl() {
  ...
  Logger.log(``);
  Logger.log(`🟣 Шаг 4: Прямая WB-выгрузка отключена; итог идёт через StreamSupps!AC...`);
  ...
  Logger.log(`⏳ Ожидание ${ARL_OZON_POSTCHECK_DELAY_MS / 1000} сек перед пост-проверкой...`);
  Utilities.sleep(ARL_OZON_POSTCHECK_DELAY_MS);

  let ozonMismatches = verifyARLOzonStocks(stocks, arlightOzonId);
  if (ozonMismatches > 0) {
    Logger.log(`⏳ Дополнительное ожидание ${ARL_OZON_POSTCHECK_RETRY_DELAY_MS / 1000} сек перед повторной проверкой...`);
    Utilities.sleep(ARL_OZON_POSTCHECK_RETRY_DELAY_MS);
    verifyARLOzonStocks(stocks, arlightOzonId);
  }
