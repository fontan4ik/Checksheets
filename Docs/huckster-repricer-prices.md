...
| колонка с заголовком `СПП X` | `market_card_discount` | скидка по карте в процентах |
| колонка с заголовком `Мин цена продажи Х` | `min_price` | минимальная цена продажи |

Новые колонки `СПП X` и `Мин цена продажи Х` находятся по заголовкам первой строки, поэтому их буквенные адреса могут быть любыми. При отсутствии или дублировании заголовка скрипт останавливается до записи. Сопоставление товаров: сначала колонка V (`SKU Ozon`), затем колонка A (`offer_id`) как fallback.
