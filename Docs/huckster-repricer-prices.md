...
Новые колонки `Цена на витрине с картой Х` и `Мин цена продажи Х` находятся по заголовкам первой строки, поэтому их буквенные адреса могут быть любыми. При отсутствии или дублировании заголовка скрипт останавливается до записи. Сопоставление товаров: сначала колонка V (`SKU Ozon`), затем колонка A (`offer_id`) как fallback.
