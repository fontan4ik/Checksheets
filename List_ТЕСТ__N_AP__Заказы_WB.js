...
function updateOrdersSummaryV2() {
  ...
  const monthSumColumn = columnByHeader_(sheet, "Сумма заказов Мес ВБ");
  const yearSumColumn = columnByHeader_(sheet, "Сумма заказо Год ВБ");
  ...
  sheet.getRange(2, monthSumColumn, colN.length, 1).setValues(colN);

  // Записываем в AP (42): Сумма заказов Год ВБ
  sheet.getRange(2, yearSumColumn, colAP.length, 1).setValues(colAP);
